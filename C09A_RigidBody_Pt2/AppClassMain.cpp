...
Application::~Application(void) 
{
	...
	// Release the RTSG singletons
	RTSG::ReleaseAllSingletons();
...
void Application::ClearScreen(RTSG::vector4 a_v4ClearColor)
{
	if (a_v4ClearColor != RTSG::vector4(-1.0f))
	{
...
void Application::Init(RTSG::String a_sApplicationName, int a_uSize, bool a_bFullscreen, bool a_bBorderless)
{
	...
	//Init System
	m_pSystem = RTSG::SystemSingleton::GetInstance();
...
void Application::Init(RTSG::String a_sApplicationName, RTSG::uint a_uWidth, RTSG::uint a_uHeight, bool a_bFullscreen, bool a_bBorderless)
{
	static bool bInitializated = false;
	if (bInitializated)
		return;
	...
	m_v4ClearColor = RTSG::vector4(RTSG::C_BLUE_CORNFLOWER, 1.0f);
	...
	//Init Camera
	m_pCameraMngr = RTSG::CameraManager::GetInstance();
	m_pCameraMngr->SetPositionTargetAndUpward(
		RTSG::vector3(0.0f, 0.0f, 15.0f),//Camera position
		RTSG::vector3(0.0f, 0.0f, 0.0f),//What I'm looking at
		RTSG::AXIS_Y);//What is up
	
	//Init Model and Entity Managers
	m_pModelMngr = RTSG::ModelManager::GetInstance();
	m_pEntityMngr = RTSG::EntityManager::GetInstance();
...
void Application::InitWindow(RTSG::String a_sWindowName)
{
	RTSG::uint uStyle = sf::Style::Default;
...
{
	RTSG::String sRoot = m_pSystem->m_pFolder->GetFolderRoot();
	RTSG::String App = m_pSystem->GetAppName();
	App = sRoot + App + ".ini";

	RTSG::FileReader reader;
	//If the file doesn't exist, exit this method
	if (reader.ReadFile(App.c_str()) == RTSG::OUT_ERR_FILE_MISSING)
	{
		return;
	}
	...
	reader.Rewind();
	while (reader.ReadNextLine() == RTSG::OUT_RUNNING)
	{
		RTSG::String sWord = reader.GetFirstWord();
		...
		else if (sWord == "FrameRate:")
		{
			...
			m_pSystem->SetMaxFrameRate(static_cast<RTSG::uint>(nValue));
		}
		...
		else if (sWord == "Background:")
		{
			...
			m_v4ClearColor = RTSG::vector4(fValueX, fValueY, fValueZ, fValueW);
		}
		else if (sWord == "Data:")
		{
...
matrix4 Application::OrientByArcball(void) { return RTSG::ToMatrix4(m_qArcBall); }