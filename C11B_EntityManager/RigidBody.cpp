...
void RigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	pointList.push_back(vector3(m_v3MinL.x, m_v3MinL.y, m_v3MinL.z));
	...
	pointList.push_back(vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MaxL.z));

	//translate them to global space
	for (uint i = 0; i < 8; i++)
	{
		pointList[i] = m_m4ToWorld * vector4(pointList[i], 1.0f);
	}

	//make a temporal rigid body for those points
	RigidBody rbTemp(pointList);

	//the new box will be oriented to the world so we get the info directly
	m_v3MinG = rbTemp.m_v3MinL;
	m_v3MaxG = rbTemp.m_v3MaxL;
...
{
	m_pMeshMngr = RTSG::ModelManager::GetInstance();
	m_bVisibleAABB = true;
	m_bVisibleARBB = true;
	m_bVisibleBS = false;
