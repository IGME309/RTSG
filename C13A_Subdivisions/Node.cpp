...
Node::Node(uint a_nSubdivisionsX, uint a_nSubdivisionsY, uint a_nSubdivisionsZ)
{
	...
	*/
	...
	//create a rigid body that encloses all the objects in this node, 
	//to subdivide the node based on will create x * y * z children. Children
...
bool Node::IsColliding(uint a_uRBIndex)
{
	...
	RigidBody* pRigidBody = pEntity->GetRigidBody();
	//Ask if the rigid body of the entity is colliding with this node
...
{
	//If this node has been subdivided already, return without changes
	if (m_uChildren != 0)
		return;
...
void Node::AssignIDstoEntities(void)
{
	...
	//Check if this node is colliding with the entity
	
	//if it is then add the uUD as the dimension for the entity
}
void Node::Init(void)
{
	m_uID = 0;
...
Node* Node::GetParent(void) { return m_pParent; }