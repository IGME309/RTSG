...
class Mesh
{
	...
	RTSG::ShaderManager* m_pShaderMngr = nullptr;	//Shader Manager
public:
	/*
...
*/
	...
	*/
	...
	/*
	USAGE: Generates a flat circle mesh
	ARGUMENTS:
	...
	*/
	...
	/*
	USAGE: Generates a collection of flat circle meshes rotating around the origin
	ARGUMENTS:
	-	float a_fRadius -> radius of the circle
	-	int a_nSubdivisions -> Number of divisions, not a lot of difference in shapes larger than 3 subd
	-	int a_nCircles -> number of circles in the shape
	-	a_v3Color -> Color of the mesh
	OUTPUT: ---
	*/
	void GenerateCCircles(float a_fRadius, int a_nSubdivisions, int a_nCircles, vector3 a_v3Color = C_WHITE);
};
